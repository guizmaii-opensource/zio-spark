...
final case class SparkContext(underlying: UnderlyingSparkContext) { self =>
  // scalafix:off
  implicit private def lift[U](x: UnderlyingRDD[U]): RDD[U] =
    RDD(x)
  implicit private def liftMap[K, V](map: scala.collection.Map[K, V]): Map[K, V] =
    map.toMap
  // scalafix:on

  /** Applies an action to the underlying SparkContext. */
  def action[U](f: UnderlyingSparkContext => U)(implicit trace: Trace): Task[U] =
    ZIO.attempt(get(f))
  ...
  def applicationAttemptId: Option[String] =
    get(_.applicationAttemptId)

  def applicationId: String =
    get(_.applicationId)

  def defaultMinPartitions: Int =
    get(_.defaultMinPartitions)

  def defaultParallelism: Int =
    get(_.defaultParallelism)
  ...
  def hadoopFile[K, V](
    ...
    get(_.hadoopFile[K, V](path, inputFormatClass, keyClass, valueClass, minPartitions))
  ...
  def hadoopFile[K, V, F <: InputFormat[K, V]](path: String, minPartitions: Int)(implicit
    ...
    get(_.hadoopFile[K, V, F](path, minPartitions))

  def hadoopFile[K, V, F <: InputFormat[K, V]](
      path: String
  )(implicit km: ClassTag[K], vm: ClassTag[V], fm: ClassTag[F]): RDD[(K, V)] =
    get(_.hadoopFile[K, V, F](path))
  ...
  def newAPIHadoopRDD[K, V, F <: NewInputFormat[K, V]](
    ...
    get(_.newAPIHadoopRDD[K, V, F](conf, fClass, kClass, vClass))
  ...
  def union[T: ClassTag](rdds: Seq[RDD[T]]): RDD[T] =
    get(_.union[T](rdds.map(_.underlying)))
  ...
  def addArchive(path: => String)(implicit trace: Trace): Task[Unit] =
    action(_.addArchive(path))

  def addFile(path: => String)(implicit trace: Trace): Task[Unit] =
    action(_.addFile(path))

  def addFile(path: => String, recursive: => Boolean)(implicit trace: Trace): Task[Unit] =
    action(_.addFile(path, recursive))

  def addJar(path: => String)(implicit trace: Trace): Task[Unit] =
    action(_.addJar(path))
  ...
  def binaryFiles(path: => String, minPartitions: => Int = defaultMinPartitions)(implicit
      trace: Trace
  ): Task[RDD[(String, PortableDataStream)]] =
    action(_.binaryFiles(path, minPartitions))

  def binaryRecords(path: => String, recordLength: => Int, conf: => Configuration = hadoopConfiguration)(implicit
      trace: Trace
  ): Task[RDD[Seq[Byte]]] =
    action(_.binaryRecords(path, recordLength, conf).map(_.toSeq))

  def broadcast[T: ClassTag](value: => T)(implicit trace: Trace): Task[Broadcast[T]] =
    action(_.broadcast[T](value))

  def cancelAllJobs(implicit trace: Trace): Task[Unit] =
    action(_.cancelAllJobs())
  ...
  def cancelStage(stageId: => Int)(implicit trace: Trace): Task[Unit] =
    action(_.cancelStage(stageId))

  def clearCallSite(implicit trace: Trace): Task[Unit] =
    action(_.clearCallSite())

  def clearJobGroup(implicit trace: Trace): Task[Unit] =
    action(_.clearJobGroup())

  def clearJobTags(implicit trace: Trace): Task[Unit] =
    action(_.clearJobTags())
  ...
  def doubleAccumulator(implicit trace: Trace): Task[DoubleAccumulator] =
    action(_.doubleAccumulator)

  def doubleAccumulator(name: => String)(implicit trace: Trace): Task[DoubleAccumulator] =
    action(_.doubleAccumulator(name))

  def getCheckpointDir(implicit trace: Trace): Task[Option[String]] =
    action(_.getCheckpointDir)

  def getExecutorMemoryStatus(implicit trace: Trace): Task[Map[String, (Long, Long)]] =
    action(_.getExecutorMemoryStatus)

  def getJobTags(implicit trace: Trace): Task[Set[String]] =
    action(_.getJobTags())

  def getLocalProperty(key: => String)(implicit trace: Trace): Task[String] =
    action(_.getLocalProperty(key))

  def getSchedulingMode(implicit trace: Trace): Task[SchedulingMode] =
    action(_.getSchedulingMode)
  ...
  def isStopped(implicit trace: Trace): Task[Boolean] =
    action(_.isStopped)
  ...
  def killTaskAttempt(
    ...
    action(_.killTaskAttempt(taskId, interruptThread, reason))

  def listArchives(implicit trace: Trace): Task[Seq[String]] =
    action(_.listArchives())

  def listFiles(implicit trace: Trace): Task[Seq[String]] =
    action(_.listFiles())

  def listJars(implicit trace: Trace): Task[Seq[String]] =
    action(_.listJars())

  def longAccumulator(implicit trace: Trace): Task[LongAccumulator] =
    action(_.longAccumulator)

  def longAccumulator(name: => String)(implicit trace: Trace): Task[LongAccumulator] =
    action(_.longAccumulator(name))
  ...
  def newAPIHadoopFile[K, V, F <: NewInputFormat[K, V]](
    ...
    action(_.newAPIHadoopFile[K, V, F](path, fClass, kClass, vClass, conf))

  def parallelize[T: ClassTag](seq: => Seq[T], numSlices: => Int = defaultParallelism)(implicit
      trace: Trace
  ): Task[RDD[T]] =
    action(_.parallelize[T](seq, numSlices))

  def register(acc: => AccumulatorV2[_, _])(implicit trace: Trace): Task[Unit] =
    action(_.register(acc))
  ...
  def runJob[T, U: ClassTag](
    ...
    action(_.runJob[T, U](rdd.underlying, func, partitions, resultHandler))

  def runJob[T, U: ClassTag](rdd: => RDD[T], func: (TaskContext, Iterator[T]) => U, partitions: => Seq[Int])(implicit
      trace: Trace
  ): Task[Seq[U]] =
    action(_.runJob[T, U](rdd.underlying, func, partitions).toSeq)
  ...
  def setCallSite(shortCallSite: => String)(implicit trace: Trace): Task[Unit] =
    action(_.setCallSite(shortCallSite))

  def setCheckpointDir(directory: => String)(implicit trace: Trace): Task[Unit] =
    action(_.setCheckpointDir(directory))
  ...
  def setJobDescription(value: => String)(implicit trace: Trace): Task[Unit] =
    action(_.setJobDescription(value))
  ...
  def setLogLevel(logLevel: => String)(implicit trace: Trace): Task[Unit] =
    action(_.setLogLevel(logLevel))

  def stop(implicit trace: Trace): Task[Unit] =
    action(_.stop())

  def stop(exitCode: => Int)(implicit trace: Trace): Task[Unit] =
    action(_.stop(exitCode))
  ...
  def textFile(path: => String, minPartitions: => Int = defaultMinPartitions)(implicit
    ...
    action(_.textFile(path, minPartitions))
