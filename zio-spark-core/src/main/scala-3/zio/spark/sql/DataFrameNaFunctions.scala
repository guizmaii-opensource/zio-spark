...
final case class DataFrameNaFunctions(underlying: UnderlyingDataFrameNaFunctions) { self =>
  ...
  def fill(valueMap: Map[String, Any]): DataFrame =
    unpack(_.fill(valueMap))

  // ===============

  def drop(cols: Seq[String]): TryAnalysis[DataFrame] =
    unpackWithAnalysis(_.drop(cols))

  def drop(how: String, cols: Seq[String]): TryAnalysis[DataFrame] =
    unpackWithAnalysis(_.drop(how, cols))

  def drop(minNonNulls: Int, cols: Seq[String]): TryAnalysis[DataFrame] =
    unpackWithAnalysis(_.drop(minNonNulls, cols))

  def fill(value: Long, cols: Seq[String]): TryAnalysis[DataFrame] =
    unpackWithAnalysis(_.fill(value, cols))

  def fill(value: Double, cols: Seq[String]): TryAnalysis[DataFrame] =
    unpackWithAnalysis(_.fill(value, cols))

  def fill(value: String, cols: Seq[String]): TryAnalysis[DataFrame] =
    unpackWithAnalysis(_.fill(value, cols))

  def fill(value: Boolean, cols: Seq[String]): TryAnalysis[DataFrame] =
    unpackWithAnalysis(_.fill(value, cols))
