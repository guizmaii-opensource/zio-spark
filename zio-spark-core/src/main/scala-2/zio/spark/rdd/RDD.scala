...
final case class RDD[T](underlying: UnderlyingRDD[T]) { self =>
   ...
   * scala.IterableOnce. Both of these functions are allowed to modify
   * and return their first argument instead of creating a new U to
   ...
   * @see
   *   <a href="https://issues.apache.org/jira/browse/SPARK-24374">
   *   SPIP: Barrier Execution Mode</a>
   * @see
   *   <a
   *   href="https://issues.apache.org/jira/browse/SPARK-24582">Design
   *   Doc</a>
   */
   ...
   *
   * The data is only checkpointed when `doCheckpoint()` is called, and
   * this only happens at the end of the first action execution on this
   * RDD. The final data that is checkpointed after the first action may
   * be different from the data that was used during the action, due to
   * non-determinism of the underlying operation and retries. If the
   * purpose of the checkpoint is to achieve saving a deterministic
   * snapshot of the data, an eager action may need to be called first
   * on the RDD to trigger the checkpoint.
   */
