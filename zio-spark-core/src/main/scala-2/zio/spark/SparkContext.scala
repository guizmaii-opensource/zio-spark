...
final case class SparkContext(underlying: UnderlyingSparkContext) { self =>
   ...
   * to group all those jobs together and give a group tag. The
   * application can use
   ...
   * parts of application may use different tags to perform cancellation
  ...
  /**
   * Add multiple tags to be assigned to all the jobs started by this
   * thread. See [[addJobTag]] for more details.
   *
   * @param tags
   ...
   * @since 4.0.0
   */
  def addJobTags(tags: => Set[String])(implicit trace: Trace): Task[Unit] = action(_.addJobTags(tags))
  ...
  /**
   * Cancel active jobs for the specified group, as well as the future
   * jobs in this job group. Note: the maximum number of job groups that
   * can be tracked is set by
   * 'spark.scheduler.numCancelledJobGroupsToTrack'. Once the limit is
   * reached and a new job group is to be added, the oldest job group
   * tracked will be discarded.
   *
   * @param groupId
   *   the group ID to cancel
   * @param reason
  ...
  /**
   * Get a read-only reference to the spark conf. This is preferred
   * version over [[getConf]].
   */
  def getReadOnlyConf(implicit trace: Trace): Task[ReadOnlySparkConf] = action(_.getReadOnlyConf)
   ...
   * thread. See [[removeJobTag]] for more details.
   *
   * @param tags
   ...
   * @since 4.0.0
   */
  def removeJobTags(tags: => Set[String])(implicit trace: Trace): Task[Unit] = action(_.removeJobTags(tags))
